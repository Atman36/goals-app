...
| `APP_ACCESS_TOKEN` | Придумайте сами (например `node -e "console.log(require('crypto').randomBytes(24).toString('base64url'))"`). Необязательна. Пока она **не** задана, замок доступа выключен и `npm run dev` работает как раньше. Если задана — приложение открывается только по ссылке `https://<адрес>/?access=<токен>` или через форму `/unlock`. Задавайте её в переменных окружения деплоя, не локально. |

> `NEXT_PUBLIC_SITE_URL` из старой auth-схемы (redirect-URL для входа) больше не используется — T9 убрал вход целиком.

> **Замок доступа — не аутентификация.** Он доказывает владение одним общим секретом и никого
> не опознаёт. Нужен только потому, что `getCurrentUser()` всегда возвращает владельца
> (`lib/auth.ts`), а значит опубликованный адрес без него отдаёт цели, суммы и фото любому,
> у кого есть ссылка. Настоящий вход появится на Этапе 1.

### 2. Миграции БД

**`drizzle-kit migrate` и `db:push` в этом репозитории запрещены.** В живой базе нет таблицы
`drizzle.__drizzle_migrations`, а `db:generate` падает на bigint-значении по умолчанию — обе
команды либо накатят уже применённое заново, либо перепишут ручной SQL. Миграции применяются
только собственным раннером, который пишет строку в реестр в **той же транзакции**, что и сам
SQL, — поэтому упавшая миграция не может быть помечена применённой:

```bash
npm run db:backup                                            # обязательно перед каждой миграцией
node --env-file=.env scripts/apply-migration.mjs drizzle/00XX_имя.sql --backup-id=<папка бэкапа>
node --env-file=.env scripts/migration-status.mjs            # сверка файлов с реестром
```

`scripts/migration-status.mjs` — релизный шлюз: он падает, если применённый файл потом
отредактировали (`CHANGED`) или если реестр знает файл, которого больше нет (`UNKNOWN`).
`PENDING` — нормальное состояние написанной, но ещё не применённой миграции.

Состояние применения определяется **только** таблицей `public.manual_migration_ledger`
(миграция `0010`), никогда — комментарием в шапке файла. Записи `0000`–`0008` внесены
задним числом по наблюдаемым следам в базе; в колонке `evidence` записано, чем именно
подтверждено каждое.
